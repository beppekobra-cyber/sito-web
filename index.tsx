...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-1000 font-lora relative overflow-hidden ${theme === 'dark' ? 'bg-obsidian text-[#F3F4F6]' : 'bg-[#FDFCF7] text-[#1C1917]'}`}>
            ...
            <div className={`absolute inset-0 z-20 flex items-center justify-center p-6 rounded-[3rem] sm:rounded-[6rem] border-2 backdrop-blur-3xl animate-fade-up ${theme === 'dark' ? 'bg-black/70 border-gold/10' : 'bg-white/70 border-[#1C1917]/10'}`}>
              <div className="max-w-xl text-center space-y-8 sm:space-y-12">
                 ...
                 <button 
                   ...
                   className={`group relative px-10 sm:px-16 py-5 sm:py-8 rounded-full overflow-hidden shadow-4xl transition-all duration-500 active:scale-95 ${theme === 'dark' ? 'gold-gradient text-black font-black uppercase tracking-widest text-[11px] sm:text-[13px]' : 'bg-[#1C1917] text-white font-black uppercase tracking-widest text-[11px] sm:text-[13px]'}`}
                 >
                    {t.ai.consentBtn}
                 </button>
              </div>
      ...
      <style>{`
        * {
        ...
        @keyframes respirate-slow {
          0%, 100% { transform: scale(1) translate(0, 0); opacity: 0.12; }
          50% { transform: scale(1.1) translate(-40px, 40px); opacity: 0.2; }
        }
        .animate-respirate {
          animation: respirate 18s ease-in-out infinite;
        }
        .animate-respirate-slow {
          animation: respirate-slow 24s ease-in-out infinite;
        }
      `}</style>
    </div>
  );
